...
- Merkle Tree building (compatible with Plonky2) using any of the above hashing methods - see [native/merkle](native/merkle).
...
Note 2: by default, the CUDA code is compiled for ``sm_89``. To change the default CUDA architeture, use ``-DCUDA_ARCH=XY`` (e.g., ``-DCUDA_ARCH=86``).
...
After that, run (in ``native`` folder):
```
...
In [gnark](https://github.com/okx/gnark), we offload some of the groth16 MSM computations to the GPU (check [backend/groth16/bn254/zeknox/zeknox.go](https://github.com/okx/gnark/blob/zeknox/backend/groth16/bn254/zeknox/zeknox.go)). Then, we benchmark the performance of proving 10 secp256r1 signatures:
...
Note: you need to install ``go`` to run the steps above:
```
