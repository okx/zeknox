...
use cryptography_cuda::{types::NTTInputOutputOrder, NTT, ntt_batch};
use rand::random;
...
fn bench_gpu_ntt(c: &mut Criterion) {
    let batches = 4;
    let LOG_NTT_SIZES: Vec<u32> = (13..=14).collect();
    ...
    for log_ntt_size in LOG_NTT_SIZES {
        ...
        group.sample_size(20).bench_function(
            ...
            |b| b.iter(|| ntt_batch(DEFAULT_GPU, &mut gpu_buffer, NTTInputOutputOrder::NN, batches as u32, log_ntt_size)),
        );
    }
}
