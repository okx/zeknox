use cryptography_cuda::{iNTT, ntt_batch,intt_batch, init_twiddle_factors_rust, types::*, NTT};
...
fn test_intt_batch_gl64_consistency_with_plonky2() {
    ...
    let mut gpu_buffer = input.clone();
    intt_batch(
        DEFAULT_GPU,
        &mut gpu_buffer,
    ...
    let plonky2_intt_input1 = input.clone();
    ...
    assert_eq!(gpu_buffer, cpu_results1);
...
#[test]
fn test_ntt_batch_gl64_self_consistency() {
    ...
    intt_batch(DEFAULT_GPU, &mut gpu_buffer, NTTInputOutputOrder::NN, 1, lg_domain_size);
