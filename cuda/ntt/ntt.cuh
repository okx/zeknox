...
protected:
     ...
     */
    static void reverse_order_batch(fr_t *arr_in, uint32_t n, uint32_t logn, uint32_t batch_size, stream_t &stream, fr_t *arr_out)
    {
     ...
     */
    static void ntt_inplace_batch_template(
      fr_t* d_input,
      int n,
      fr_t* d_twiddles,
      int n_twiddles,
...
public:
     ...
     */
    static RustError Batch(const gpu_t &gpu, fr_t *inout, uint32_t lg_domain_size, uint32_t batch_size,
                           InputOutputOrder order, Direction direction,
                           Type type, bool coset_ext_pow = false, bool are_outputs_on_device = false)
    {
        ...
        try
        {
            ...
            if (reverse_input) reverse_order_batch(d_input, size, lg_domain_size, batch_size, gpu, d_output);
