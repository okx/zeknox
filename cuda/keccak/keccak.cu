...
__device__ void keccakf(uint64_t st[25], int rounds)
{
    int i, j, round;
    uint64_t t, bc[5];
...
__device__ void keccak(const uint8_t *in, int inlen, uint8_t *md, int mdlen)
{
    uint64_t st[25];    
    uint8_t temp[144];
    int i, rsiz, rsizw;
...
{
    keccak((uint8_t*)inputs, num_inputs * 8, (uint8_t*)digest, 32);
    hash[3] &= 0xFF;
}
...
{
    uint64_t input[8];
    input[0] = hash1[0].get_val();
    input[1] = hash1[1].get_val();;
    input[2] = hash1[2].get_val();;
    input[3] = hash1[3].get_val();;
    input[4] = hash2[0].get_val();;
    input[5] = hash2[1].get_val();;
    input[6] = hash2[2].get_val();;
    input[7] = hash2[3].get_val();;
