...
/**
* @brief this kernel splits the scalars into digits of size c
* each thread splits a single scalar into nof_bms digits
* @param[in] total_size, total msm size
* @param[in] msm_log_size, ceil(log2(total_size))
*/
template <typename S>
...
/**
 * @brief this kernel adds up the points in each bucket
 * @param[in] buckets, contains point for each bucket, size is num_of_windows * window_size (1<<c)
 * @param[in] bucket_offsets, size is num_of_buckets_to_compute, offset points to a group of points belong to the same bucket
 * @param[in] bucket_sizes, size is num_of_buckets_to_compute, indicate the size of each bucket (ie. number of points in each bucket)
 * @param[in] single_bucket_indices, size is same as num_of_buckets_to_compute, bucket indice
 * @param[in] point_indices, size is num_of_windows * msm_size, the value is an indice to the corresponding point 
 * @param[in] points, pointer to the device raw point data, size is msm_size
 * @param[in] nof_buckets, size is num_of_window * window_size(1<<c)
 * @param[in] msm_idx_shift, c+bm_bits, c is number of bits per window. bm_bits is bits for num_of_windows, if num_of_windows=16, bm_bits=4
 */ 
...
/** 
 * @brief reduction kernel to adds up all buckets within one window
 * in each call, it reduce by a factor of 2.
*/
template <typename P>
...
/**
 * @brief sum over all windows
 * // this kernel computes the final result using the double and add algorithm
// it is done by a single thread 
* @param[in] final_sums, sums of each window, 
* @param[in] final_results, the final result, as it is single threaded. so the size is one. 
*/
...
/**
 * @brief it is similar to accumulate_buckets_kernel. it is customized for large buckets
*/
template <typename P, typename A>
