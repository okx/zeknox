...
RustError mult_pippenger_g2(uint32_t device_id,
                            ...
                            MSM_Config cfg)
{
    printf("start mult_pippenger_g2, device_id: %d, npoints: %d, are_inputs_on_device: %d, big_triangle:%d, large_bucket_factor: %d \n", device_id, cfg.npoints, cfg.are_inputs_on_device, cfg.big_triangle, cfg.large_bucket_factor);
    ...
    for(int i=0; i<cfg.npoints; i++ ) {
        *(scalar+i) = scalar_field_t::from_montgomery(*(scalar+i));
        *(points+i) = g2_affine_t::from_montgomery(*(points+i));
    }
    ...
    RustError r = mult_pippenger_g2_internal<scalar_field_t, g2_projective_t, g2_affine_t>(
        result_jac, points, scalar, cfg.npoints, false, false, 2);
    ...
    uint32_t *gpu_result_proj_limbs = gpu_result_affine_mont.x.real.export_limbs();
    // printf("gpu result affine mont \n");
    // printf("x.real \n");
    // for (int i = 0; i < 8; i++)
    // {
    //     printf("%x", *(gpu_result_proj_limbs + i));
    // }
    // printf("\n x.imaginery \n");
    // gpu_result_proj_limbs = gpu_result_affine_mont.x.imaginary.export_limbs();
    // for (int i = 0; i < 8; i++)
    // {
    //     printf("%x", *(gpu_result_proj_limbs + i));
    // }
    ...
    // printf("\n y.real \n");
    // gpu_result_proj_limbs = gpu_result_affine_mont.y.real.export_limbs();
    // for (int i = 0; i < 8; i++)
    // {
    //     printf("%x", *(gpu_result_proj_limbs + i));
    // }
    ...
    // printf("\n y.imaginery \n");
    // gpu_result_proj_limbs = gpu_result_affine_mont.y.imaginary.export_limbs();
    // for (int i = 0; i < 8; i++)
    // {
    //     printf("%x", *(gpu_result_proj_limbs + i));
    // }
