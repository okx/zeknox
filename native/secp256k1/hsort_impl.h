...
/* In-place heap sort. */
SECP256K1_FUNC static void secp256k1_hsort(void *ptr, size_t count, size_t size,
                            int (*cmp)(const void *, const void *, void *),
                            void *cmp_data) {
    size_t i;

    for (i = count/2; 0 < i; --i) {
        secp256k1_heap_down((unsigned char *)ptr, i-1, count, size, cmp, cmp_data);
    }
    for (i = count; 1 < i; --i) {
        /* Extract the largest value from the heap */
        secp256k1_heap_swap((unsigned char *)ptr, 0, i-1, size);

        /* Repair the heap condition */
        secp256k1_heap_down((unsigned char *)ptr, 0, i-1, size, cmp, cmp_data);
    }
}
